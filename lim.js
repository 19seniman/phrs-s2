...
// Function to build a fallback provider for reliable RPC connection
...
const colors = {
  ...
  blue: "\x1b[34m",
  magenta: "\x1b[35m",
  gray: "\x1b[90m" // Added gray for the new logger
};
...
const logger = {
    ...
    countdown: (msg) => process.stdout.write(`\r${colors.blue}[⏰] ${msg}${colors.reset}`), // Kept for countdown functionality
    banner: () => {
...
async function craftTokens(wallet) {
  ...
  try {
    ...
    if (cAllowance < requiredAmount) {
        logger.info('Approving C tokens...');
    ...
    if(sAllowance < requiredAmount) {
        logger.info('Approving S tokens...');
    ...
    logger.loading("Crafting CS tokens...");
  ...
  while (true) {
    for (const [index, privateKey] of privateKeys.entries()) {
      try {
        ...
        logger.section(`Processing Wallet ${index + 1}/${privateKeys.length}: ${wallet.address}`);
