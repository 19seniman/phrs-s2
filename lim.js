...
async function swapUSDCToR2USD(wallet, amount) {
 ...
 try {
   ...
   try {
     ...
     logger.success(`Swap TX (0x095e7a95 templated) sent: ${tx.hash}`);
     const rc = await tx.wait();
   ...
   logger.success(`Swap TX (mint) sent: ${tx2.hash}`);
...
/**
 * Waits for a specified number of hours and displays a countdown.
 * @param {number} hours - The number of hours to wait.
 */
async function waitAndCountdown(hours) {
    const waitMilliseconds = hours * 60 * 60 * 1000;
    const endTime = Date.now() + waitMilliseconds;

    logger.info(`Semua dompet telah diproses. Siklus berikutnya akan dimulai dalam ${hours} jam.`);
    ...
    return new Promise(resolve => {
        const interval = setInterval(() => {
            ...
            if (remaining <= 0) {
                ...
                logger.step("Memulai siklus berikutnya...");
                resolve();
            } else {
                ...
                logger.countdown(`Siklus berikutnya dalam: ${h} jam ${m} menit ${s} detik`);
            }
        }, 1000);
    });
}
...
(async () => {
 ...
 while (true) {
   for (const [i, pk] of privateKeys.entries()) {
     ...
     logger.success(`Processing Wallet ${i + 1}/${privateKeys.length}: ${wallet.address}`);
     console.log("-----------------------------------------------------");
   ...
   // Wait for 24 hours before starting the next cycle for all wallets.
   await waitAndCountdown(24);
 }
})().catch((err) => {
